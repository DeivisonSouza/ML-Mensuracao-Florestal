...
.pull-left-2[
```{r, echo=FALSE, fig.width=5, fig.height=3, fig.cap=""}

prebag <- 1:length(bagTr$mtrees)
results <- data.frame(NULL)

for(i in prebag) {
  pred <- mutate(data, predbag = paste("Tree",prebag[i]))
  pred$pred <- predict(bagTr$mtrees[[i]]$btree, newdata=data)
  results <- rbind(results, pred) %>%
