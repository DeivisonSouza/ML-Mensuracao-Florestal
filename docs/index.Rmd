...
.panel[.panel-name[Dados]
.pull-left-2[
```{r, echo=FALSE, fig.align='center', fig.width=4, fig.height=2, fig.cap='', dpi=600, eval=F, warning=FALSE, message=FALSE}
library(ipred)
data <- data.table::fread("R-scripts/Cedrela.csv")
data2 <- data %>% select(-Arv)
...
bg1 <- rpart.plot(bagTr$mtrees[[1]]$btree, 
                  box.palette="RdBu", 
                  shadow.col="gray", 
                  nn=TRUE, 
                  digits = 4)

bg2 <- rpart.plot(bagTr$mtrees[[2]]$btree, 
                  box.palette="RdBu", 
                  shadow.col="gray", 
                  nn=TRUE, 
                  digits = 4)

bg3 <- rpart.plot(bagTr$mtrees[[3]]$btree, 
                  box.palette="RdBu", 
                  shadow.col="gray", 
                  nn=TRUE, 
                  digits = 4)

bg4 <- rpart.plot(bagTr$mtrees[[4]]$btree, 
                  box.palette="RdBu", 
                  shadow.col="gray", 
                  nn=TRUE, 
                  digits = 4)

# ggpubr::ggarrange(bg1, bg2, bg3, bg4, ncol=4, 
#                   labels=c("", ""),
...
**4 - Covariáveis tradicionais**: Usar apenas as covariáveis tradicionais (ex.: D e H para modelar o volume) para estimar modelos de AM parece não produzir melhores resultados do que os métodos tradicionais (ex.: Schumacher-Hall). Sobretudo, em florestas equiâneas, em FNI ainda faltam mais estudos.
